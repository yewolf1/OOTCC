...
from core.dll_runtime_symbols import DllRuntimeSymbolResolver
...
class MagicButtonsTeleportControllerMixin:
    ...
    def _dll_symbols(self) -> DllRuntimeSymbolResolver:
        resolver = getattr(self, "_dll_runtime_symbol_resolver", None)
        if not isinstance(resolver, DllRuntimeSymbolResolver):
            resolver = DllRuntimeSymbolResolver(self, self._log)
            setattr(self, "_dll_runtime_symbol_resolver", resolver)
        return resolver

    def _resolve_dll_runtime_symbols(self, force_refresh: bool = False) -> dict[str, int]:
        return self._dll_symbols().resolve(force_refresh=force_refresh)
